...
+ A crucial function of the `TMEM` package is the `get_GO_info` function that 
gathers **ALL** the gene ontology information of a list of gene or protein 
identifiers
...
website; this pre-dated the `clusterProfiler` package

+ The `get_GO_info` function gathers all the relevant information in one place 
without needing to generate multiple files and analyses across 1+ browsers-- 
it's all in-house

+ However, there are (unsurprisingly) discrepancies between `geneontology.org`'s
annotations and `Bioconductor`'s

+ Furthermore, `Bioconductor` does not employ `geneontology.org`'s gene ontology 
...
package for publication statistics
...
+ *I recommend passing the list of genes with their aliases*, 
...
+ This is highly useful for generating heatmaps
...
+ Interested in finding genes/proteins related to a certain cellular phenomenon?

+ When there are multiple terms involving a certain `GO` phrase (i.e. 
`organelles` or `processes`), search `GO` terms with regular expressions!
...
+ Similarly, the results returned by the `get_GO_info` function include all the
information on all the `GO Term`s-- not just including the genes/proteins from
the list of interest that are associated with a `GO Term` (these IDs are called
"`Overlap`")

+ It provides *all* the IDs associated with the `GO Term`
